...
@section('content')
    <div class="mb-6 text-center">
        <h2 class="text-2xl font-black text-gray-900">Crie sua conta</h2>
        <p class="text-sm text-gray-600 mt-2">Cadastre-se para aproveitar as melhores ofertas</p>
    </div>
    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        <div>
            ...
            <input id="name" class="block mt-1 w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name" />
            @error('name')
        ...
        <div class="mt-4">
            ...
            <input id="email" class="block mt-1 w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" type="email" name="email" value="{{ old('email') }}" required autocomplete="username" />
            @error('email')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mt-6 text-center">
            <p class="text-sm text-gray-600">
                Já tem uma conta?
                <a href="{{ route('login') }}" class="font-bold text-indigo-600 hover:text-indigo-900">
                    Entrar
                </a>
            </p>
        </div>
