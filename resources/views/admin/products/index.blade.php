                        ...
                        @forelse ($products as $product)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div class="text-xs text-gray-500">{{ Str::limit($product->affiliate_url, 30) }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('products.show', $product) }}" target="_blank" class="text-gray-400 hover:text-gray-600 mr-3">Ver</a>
                                    <a href="{{ route('admin.products.edit', $product) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Editar</a>
                                    <form action="{{ route('admin.products.destroy', $product) }}" method="POST" class="inline-block" onsubmit="return confirm('Tem certeza?');">
                                        @csrf
                                        @method('DELETE')
